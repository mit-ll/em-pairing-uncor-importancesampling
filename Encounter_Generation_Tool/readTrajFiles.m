...
dpsi = computeHeadingRate(results(1).psi_rad,own_time_s(1 : end)); %rad/sec
results(1).phi_rad = atan(results(1).speed_ftps.*dpsi/constants.g);
...
dpsi = computeHeadingRate(results(2).psi_rad,int_time_s(1 : end)); %rad/sec
results(2).phi_rad = atan(results(2).speed_ftps.*dpsi/constants.g);
