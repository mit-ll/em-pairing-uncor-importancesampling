...
% n_initial = numel(labels_initial);
% n_transition = numel(labels_transition);
% n_dynamic = n_transition - n_initial;
% temporal_map = [(n_initial - n_dynamic + 1):n_initial; (n_initial + 1):n_transition]';
tt = [];
tt1 = [];
for i = 1:numel(labels_transition);
    t = findstr(labels_transition{i}, '(t)');
    if isempty(t)
        t = findstr(labels_transition{i}, '(t+1)');
        if ~isempty(t)
            tt1 = [tt1; i]; %#ok<AGROW>
        end
    else
