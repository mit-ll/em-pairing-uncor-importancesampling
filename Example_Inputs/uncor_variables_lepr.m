...
function [G_initial, r_initial, G_transition, r_transition, temporal_map, zero_bins, labels_initial, labels_transition, boundaries, resample_rates] = ... 
  uncor_variables_lepr()
% UNCOR_VARIABLES_LEPR returns variables for the uncorrelated encounter
% model modified for for a generic LEPR aircraft.
%
...
%       uncor_variables();
%
% See also LEPR_UNCOR_STATISTICS
...
bounds_initial = [0 0; ... % Region
                  0 0; ... % Airspace
                  0 0; ... % Layer
                  40 100; ... % Airspeed (KTAS)
                  -1 1; ... % Airspeed Acceleration (kt/s^2)
                  -500 500; ... % Vertical Rate (ft/min)
                  -2.5 2.5]; % Turn Rate (deg/s)

zero_bins = { [], [], [], [], 1, 2, 4 }; % The location (index) of the bin where the sampled values are 0
