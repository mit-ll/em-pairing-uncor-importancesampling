...
#define	I2_in	prhs[2] /* initial AC2 */
#define C2_in   prhs[3] /* controls AC2 */
#define R_in    prhs[4] /* runtime */
#define opt_in  prhs[5] /* options in [nmac/enc cylinder break flag,renc,hend] */
...
#define qmax    1.5*M_PI/180  /* As in DEGAS [rad/s]     */
#define rmax    3*M_PI/180     /* As in DEGAS GA_psidotMAX = 1e6; */
#define phi_max 75*M_PI/180
#define phidotmax       0.524 
#define v_high  1116      /* Airspeed limits */
#define v_low   1.7
#define num_ac  2           /* Number of Aircraft */
#define dh_ftps_max    25  /* Vertical Rate limits */
#define dh_ftps_min    -25  /* Vertical Rate limits */
...
{    
    double      s_theta,c_theta,t_theta, /* Trig. values of Euler angles */
                s_phi,c_phi,
                s_psi,c_psi,
    ...
    dhcmd = MAX( MIN( dh_ftps_max, dhcmd ), dh_ftps_min ); /*(03.19.2020 Updated to include vertical rate saturation - CCS)*/
    
    /* resolve TCAS and script */
    hd = x[col_v]*s_theta;
    hddcmd = 1/dt*(dhcmd-hd); /* May want to saturate as in DEGAS */    
    
    /* Compute and saturate q (08.25.2011 Updated to include airspeed saturation - MWE) */
    ...
    if(x[col_phi]+p*dt < -phimax)
        p = (-phimax - x[col_phi])/dt;  /*JDG - added negative on 9/17/07 */
...
{ 
    ...
    unsigned int i,istop,j,     /* Dummy indicies */
            cmd_i,cmd_i2,   /* Command indicies */
    ...
    if (nrhs < 5) {
        mexErrMsgTxt("More than four input arguments required.");        
    }
    if(nrhs==6){ /* If input parameters specified */
        if(mxGetN(opt_in)<6)
    ...
    for(i=0;i<(int)nvalues;i++) /* Loop over all time */
    {
        ...
        {            
            ...
            degas(x,ptrc,cmd_i,c_m);      /* Run dynamics AC1 */
            degas(x2,ptrc2,cmd_i2,c_m2);  /* Run dynamics AC2             */
        }
