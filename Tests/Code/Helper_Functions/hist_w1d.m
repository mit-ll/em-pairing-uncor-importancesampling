...
function [histx, histy, sumyw] = hist_w1d(data, varargin)
    % This function will give the weighted distribution given the inputs; if no
    % outputs, will plot the values with optional labelx and labely

    % Parse optional inputs
    opts = inputParser;

    opts.addOptional('w', ones(size(data)), @(x)all([isnumeric(x), size(x) == size(data)])); % weights
    ...
    opts.addOptional('smooth', false, @islogical);        % Boolean to smooth final density (using normal kernal)
    ...
    opts.addOptional('linestyle', '-', @ischar);
    opts.addOptional('Color', 'k');
    opts.addOptional('plotresults', logical(nargout), @islogical); % Plot results?
    ...
    [histy_uw, histy_bins] = histc(data, bins);
    ...
    if opts.Results.plotresults
        plot(histx, histy, opts.Results.linestyle, 'Color', opts.Results.Color);
